...
export function listCapabilityGaps(sourceId:number,scope:"knowledge"|"all"="knowledge"){return request<CapabilityGapBoard>(`/capability-gaps?sourceId=${sourceId}&scope=${scope}`);}
...
export function getOntologyActivation(sourceId:number,versionId:number){return request<import("./types").OntologyActivationStatus>(`/sources/${sourceId}/ontology-build/activation?versionId=${versionId}`);}
export function activateOntologyVersion(sourceId:number,input:{versionId:number;mode:"activate"|"retain_removed_properties";snapshotChecksum:string}){return request<{task:BackgroundTask;versionId:number;version:number;reused:boolean}>(`/sources/${sourceId}/ontology-build/activate`,{method:"POST",headers:writeHeaders,body:JSON.stringify(input)},20_000);}
...
export function resumeSourceOntology(sourceId:number,input:{taskId:string;answers?:import("./types").OntologyBuildAnswer[];approveChangeChecksum?:string;retryLinkGeneration?:boolean;retryVerification?:boolean}){return request<BackgroundTask>(`/sources/${sourceId}/ontology-build/resume`,{method:"POST",headers:writeHeaders,body:JSON.stringify(input)},20_000);}
export function correctSourceOntology(sourceId:number,input:{versionId:number;objectName:string;propertyName?:string;displayName:string;description:string}){return request<BackgroundTask>(`/sources/${sourceId}/ontology-build/correct`,{method:"POST",headers:writeHeaders,body:JSON.stringify(input)},20_000);}
...
export function previewSourceTables(sourceId:number){return request<{tables:TableSelectionRow[]}>(`/sources/${sourceId}/tables/preview`,{},30_000);}
export function saveTableSelections(sourceId:number,selections:Array<{tableName:string;included:boolean}>){return request<{ok:boolean;saved:number;purgedTables:number;excluded:number}>(`/sources/${sourceId}/tables/selection`,{method:"PUT",headers:writeHeaders,body:JSON.stringify({selections})},30_000);}
...
export function runEvaluation(sourceId:number,setName:string){return request<BackgroundTask>("/eval/run",{method:"POST",headers:writeHeaders,body:JSON.stringify({sourceId,setName,tolerance:1e-6})},20_000);}
