import type { BackgroundTask, BootstrapData, ConnectionTestInput, ConnectionTestResult, DataSource, DiscoverySummary, EvalCase, EvalInput, KnowledgeInput, KnowledgePage, OntologyBulkDecisionResult, OntologyCalibrationGate, OntologyCalibrationLabel, OntologyCalibrationReport, OntologyCandidate, OntologyCandidateEvent, OntologyConflictResolution, OntologyDraftApplyResult, OntologyDraftPreview, OntologyDomainPlan, OntologyGenerationRun, OntologyGenerationScopePlan, OntologyGenerationTraceDetail, OntologyGenerationTraceSummary, OntologyCatalog, QueryResponse, QuerySession, QuerySessionDetail, QueryStreamEvent, RelationDocument, SemanticLinkType, SemanticObjectType, SemanticSchema, SemanticSchemaDiff, SemanticSchemaValidation, SemanticSchemaVersion, SettingsData, SettingsInput, SourceInput, TermAnchor } from "./types";
...
export function startOntologyDomainModeling(sourceId:number,options:{refreshDomainPlan?:boolean;domainIds?:string[]}={}){return request<BackgroundTask>("/ontology/domain-modeling",{method:"POST",headers:writeHeaders,body:JSON.stringify({sourceId,refreshDomainPlan:options.refreshDomainPlan!==false,domainIds:options.domainIds||[]})},20_000);}
...
export function listOntologyGenerationRuns(sourceId:number){return request<OntologyGenerationRun[]>(`/ontology/generation-runs?sourceId=${sourceId}`);}
