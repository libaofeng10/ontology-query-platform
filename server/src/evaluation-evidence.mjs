...
export function inspectSemanticEvalGate(gate,{sourceId,schemaVersion,schemaPublishedAt=null,requirePostPublish=false,cases=[]}={}) {
  ...
  if(!gate.passed||gate.decision!=="enable_claude")issues.push(issue("not_passed","Gold SQL 语义对照门禁尚未通过"));
  if(gate.baseline?.requestedMode!=="gold"||gate.candidate?.requestedMode!=="claude")issues.push(issue("wrong_gate_kind","必须使用 Claude Code 对照 Gold SQL 的门禁"));
  ...
  if(!(Number(gate.candidate?.claudeExecutionRate)>0))issues.push(issue("claude_not_executed","候选组没有实际执行 Claude Code"));
