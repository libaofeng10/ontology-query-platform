  ...
  return {
    ...
    purgeExcludedTables(sourceId) {
      ...
      db.transaction(()=>{
        ...
        // A JOIN question names the excluded table on its RELATION's far end while its own
        // table_name points at the surviving side, so the per-table sweep above misses it.
        // Runs unconditionally (not only when a table was removed this call): the tables may
        // have been purged by an earlier call that predates this cleanup, and re-saving the
        // selection must heal those orphans too.
        db.prepare(`UPDATE ds_question SET status='obsolete',answered_at=CURRENT_TIMESTAMP WHERE source_id=? AND status='pending' AND relation_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM ds_relation r WHERE r.id=ds_question.relation_id AND r.present=1)`).run(sourceId);
      })();
      return removed;
    },
    ...
    listTasks(sourceId,limit=50) { return db.prepare(`SELECT id,source_id AS sourceId,task_type AS taskType,status,progress,total,current_step AS currentStep,payload_json AS payloadJson,result_json AS resultJson,error,created_at AS createdAt,started_at AS startedAt,finished_at AS finishedAt,updated_at AS updatedAt FROM ds_task WHERE source_id=? ORDER BY created_at DESC,rowid DESC LIMIT ?`).all(sourceId,limit).map(parseTask); },
    ...
    updateTaskPayload(id,payload) { db.prepare(`UPDATE ds_task SET payload_json=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND status='running'`).run(JSON.stringify(payload),id); },
