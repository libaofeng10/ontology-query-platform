  ...
  return {
    ...
    upsertTable(table) {
      db.prepare(`INSERT INTO ds_table (source_id, table_name, row_estimate, grade, grade_override, active, last_probe_at, comment, days_since_write, inbound_relations, present)
        ...
        ON CONFLICT(source_id,table_name) DO UPDATE SET row_estimate=excluded.row_estimate, grade=CASE WHEN ds_table.grade_override IS NOT NULL THEN ds_table.grade_override ELSE excluded.grade END, active=excluded.active, present=1, last_probe_at=CURRENT_TIMESTAMP, comment=excluded.comment, days_since_write=excluded.days_since_write, inbound_relations=excluded.inbound_relations`).run({ gradeOverride:null, active:1, comment:null, daysSinceWrite:null, inboundRelations:0, rowEstimate:0, ...table });
    },
    upsertColumn(column) {
