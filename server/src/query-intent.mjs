...
export function parseQueryIntent(question,{now=new Date(),concepts=[],filterConcepts=[],rowDomainConcepts=[],protectedTermAliases=[],timeZone}={}) {
  ...
  for(const measure of measures.filter((item)=>item.aggregation==="ratio"&&item.evidence?.level!=="verified_knowledge"&&(item.definitionCandidates||[]).length<=1))ambiguities.push({code:"MEASURE_DEFINITION_REQUIRED",message:`比例指标“${measure.sourceText||measure.value}”缺少已验证的分子、分母和去重口径`,blocking:true,sourceText:measure.sourceText||measure.value,options:["选择已登记指标口径","先补充指标定义"]});
  for(const measure of measures.filter((item)=>(item.definitionCandidates||[]).length>1))ambiguities.push({code:"METRIC_AMBIGUOUS",message:`“${measure.sourceText||measure.value}”匹配到多个已验证指标定义`,blocking:true,sourceText:measure.sourceText||measure.value,options:measure.definitionCandidates.map((item)=>item.title||item.source)});
