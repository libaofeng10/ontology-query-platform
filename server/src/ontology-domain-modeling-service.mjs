export function createOntologyDomainModelingService({domainPlanner,candidates}={}) {
  ...
  function assertReady(sourceId) {
    const active=candidates.listRuns(sourceId,500).find((run)=>["queued","running"].includes(run.status));
  ...
  async function run({task,source,payload,onProgress=()=>{}}={}) {
    ...
    const priorRuns=candidates.listRuns(source.id,500).filter((run)=>run.scope?.orchestrationId===orchestrationId);
    const results=[];
