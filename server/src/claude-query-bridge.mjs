...
function buildPrompt(request = {}, snapshot = null) {
  ...
  const lines = [
    ...
    "用户同时提供机构/团队名称和一个 ID 时，把它们作为同一实体的线索。团队字样不等于该 ID 是 team_id；先依据目标产品的业务定义及所属机构/系统字段选择 ID 归属，不要依次猜团队 ID、用户 ID、管理员 ID。目标用户表已有机构名称和机构 ID 字段时，优先用两者约束所需明细；若归属不确定，用名称与 ID 一起核对机构记录，定位结果必须能解释原始名称。",
    "生成 SQL 前核对字段的物理名称；不确定时用 get_tables 读取，不要把其他表的同义字段搬过来。一次定位未命中后结合名称和字段含义调整，不连续消耗预算试同一个 ID。每次结果中的 queryBudget 给出剩余额度，优先留给目标明细。",
  ...
  if (budget) lines.push(`本轮 SQL 预算：${budget.maxSqlCalls} 次，普通失败与空结果消耗预算。数据库确认的表列错误另有最多 ${budget.maxSchemaRepairs || 0} 次修正机会，按工具返回的结构和剩余额度处理。`);
