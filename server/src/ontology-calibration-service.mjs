...
export function createOntologyCalibrationService({store,config,settings}={}) {
  ...
  function report(sourceId,input={}) {
    ...
    const globalAutoThreshold=boundedInteger(config?.ontologyAi?.autoConfirmScore,0,100,85);const sourceSetting=store.getSourceOntologySetting?.(source.id)||null;const autoThreshold=boundedInteger(sourceSetting?.autoConfirmScore,0,100,globalAutoThreshold);
    ...
    return {
      ...
      conditions,passed,decision:passed?"enable_auto_draft":"keep_review",reason:passed?"质量校准全部通过。":`质量校准尚未通过：${failedConditions.join("、")}尚未满足；当前确认方式不变。`,labels,
    };
  }
