...
export function createApp(overrides={}) {
  ...
  async function handler(req,res) {
    ...
    try {
      ...
      const answerMatch=url.pathname.match(/^\/api\/questions\/(\d+)\/answer$/);if(req.method==="POST"&&answerMatch){const body=await readJson(req);if(!body.answer)throw badRequest("answer 必填");const question=store.getQuestion(Number(answerMatch[1]));if(!question)throw notFound("问题不存在");authorize(identity,"editor",question.sourceId);const changes=store.answerQuestion(question.id,String(body.answer),identity.name);if(question.kind==="JOIN 路径"&&question.relationId){if(/^确认/.test(body.answer))store.setRelationStatus(question.relationId,"confirmed");else if(/^不允许/.test(body.answer))store.setRelationStatus(question.relationId,"denied");}else if(question.kind==="JOIN 路径"&&/^确认/.test(body.answer))store.confirmRelationByColumn(question.sourceId,question.tableName,question.columnName);if(question.scope==="global"&&/^全部/.test(body.answer))store.addRule({sourceId:question.sourceId,name:question.question.replace(/[？?]/g,""),content:`字段族 ${question.columnName||"amount"} 统一按分存储，查询展示时除以 100.0`,appliesTo:question.tableName,verified:1});await discovery.writeOntology(question.sourceId);return send(res,200,{ok:Boolean(changes),remaining:store.listQuestions(question.sourceId).length});}
      ...
      if(req.method==="POST"&&url.pathname==="/api/ontology/domain-modeling"){const body=await readJson(req);const sourceId=requiredSourceId(store,identity,body.sourceId,"editor");const active=store.findActiveTask(sourceId,"ontology_domain_modeling");if(active)return send(res,202,active);ontologyDomainModeling.assertReady(sourceId);return send(res,202,tasks.create({sourceId,taskType:"ontology_domain_modeling",payload:{refreshDomainPlan:body.refreshDomainPlan!==false,domainIds:Array.isArray(body.domainIds)?body.domainIds:[],actor:identity.name}}));}
      ...
      if(req.method==="GET"&&url.pathname==="/api/ontology/generation-runs"){const sourceId=requiredSourceId(store,identity,url.searchParams.get("sourceId"));return send(res,200,ontologyCandidates.listRuns(sourceId));}
  ...
  async function streamQuery(req,res,{body,source,identity}) {
    ...
    try {
      const rawResult=await queries.ask({sourceId:source.id,question:body.question,sessionId:body.sessionId,pendingId:body.pendingId,userName:identity.name,signal:controller.signal,onEvent:(event)=>writeSse(res,event.type,event)});
...
function lockedSettingKeys(overrides){const keys=[];for(const group of ["llm","embedding","retrieval","profiling","ontologyAi"])if(overrides[group])for(const key of Object.keys(overrides[group]))keys.push(`${group}.${key}`);for(const key of ["semanticQueryPlanMode","queryAgentMode","queryAgentTrafficPercent","queryAgentMaxIterations","queryAgentMaxSqlCalls","queryAgentMaxScannedRows","queryAgentPendingTtlMs","queryMaxRows","explainMaxRows","queryTimeoutMs","queryLlmTimeoutMs"])if(key in overrides)keys.push(`query.${key}`);return keys;}
...
function badRequest(message){const error=new Error(message);error.status=400;return error;}function forbidden(message){const error=new Error(message);error.status=403;return error;}function notFound(message){const error=new Error(message);error.status=404;return error;}
