...
export function createApp(overrides={}) {
  ...
  const store=overrides.store||createStore(runtime.dbPath);
  ...
  const capabilityGaps=createCapabilityGapService({store,currentPlanningMode:()=>"claude"});
  ...
  const queries=createQueryService({store,connector,config:settingsConfig,claudeBridge,claudeMcpFactory:overrides.claudeMcpFactory});
  ...
  async function handler(req,res) {
    ...
    try {
      ...
      if(req.method==="POST"&&sourceBuildAction){const source=requiredSource(store,identity,sourceBuildAction[1],"editor");const body=await readJson(req);return send(res,202,await sourceOntologyBuild[sourceBuildAction[2]](source,body,identity.name));}
      const sourceDiscover=url.pathname.match(/^\/api\/sources\/(\d+)\/discover$/);if(req.method==="POST"&&sourceDiscover){const source=requiredSource(store,identity,sourceDiscover[1],"editor");if(source.lastTestOk!==1)throw badRequest("真实数据源必须先通过只读连接测试");const active=store.findActiveTask(source.id,"discovery");if(active)return send(res,202,active);sourceOntologyBuild.assertCatalogIdle(source.id);return send(res,202,tasks.createDiscoveryTask(source,{resumeRelations:url.searchParams.get("resume")==="true"}));}
      const sourceTablePreview=url.pathname.match(/^\/api\/sources\/(\d+)\/tables\/preview$/);if(req.method==="GET"&&sourceTablePreview){const source=requiredSource(store,identity,sourceTablePreview[1],"editor");if(source.lastTestOk!==1)throw badRequest("真实数据源必须先通过只读连接测试");return send(res,200,{tables:await discovery.previewTables(source)});}
      const sourceTableSelection=url.pathname.match(/^\/api\/sources\/(\d+)\/tables\/selection$/);if(req.method==="PUT"&&sourceTableSelection){const source=requiredSource(store,identity,sourceTableSelection[1],"editor");const body=await readJson(req);sourceOntologyBuild.assertCatalogIdle(source.id);if(!Array.isArray(body.selections)||!body.selections.length)throw badRequest("selections 必须是非空数组");for(const item of body.selections){if(typeof item?.tableName!=="string"||!item.tableName.trim())throw badRequest("selections 每一项必须带 tableName");}store.saveTableSelections(source.id,body.selections.map((item)=>({tableName:item.tableName.trim(),included:item.included!==false})),identity.name);const purged=store.purgeExcludedTables(source.id);if(purged)await discovery.writeOntology(source.id);return send(res,200,{ok:true,saved:body.selections.length,purgedTables:purged,excluded:[...store.excludedTableNames(source.id)].length});}
      ...
      if(req.method==="GET"&&url.pathname==="/api/knowledge"){const sourceId=requiredSourceId(store,identity,url.searchParams.get("sourceId"));return send(res,200,knowledge.listBusiness(sourceId));}
      ...
      if(req.method==="POST"&&url.pathname==="/api/knowledge"){const body=await readJson(req);const sourceId=requiredSourceId(store,identity,body.sourceId,"editor");return send(res,201,await knowledge.save(sourceId,{...body,owner:identity.name}));}
      ...
      if(req.method==="POST"&&url.pathname==="/api/ontology/domain-modeling"){const body=await readJson(req);const sourceId=requiredSourceId(store,identity,body.sourceId,"editor");const active=store.findActiveTask(sourceId,"ontology_domain_modeling");if(active)return send(res,202,active);sourceOntologyBuild.assertCatalogIdle(sourceId);ontologyDomainModeling.assertReady(sourceId);const orchestrationId=String(body.orchestrationId||"").trim()||null;if(orchestrationId){const original=store.getTask(orchestrationId);if(!original||original.taskType!=="ontology_domain_modeling"||Number(original.sourceId)!==Number(sourceId))throw badRequest("待恢复的全域建模任务不存在或不属于当前数据源");}return send(res,202,tasks.create({sourceId,taskType:"ontology_domain_modeling",payload:{refreshDomainPlan:body.refreshDomainPlan!==false,domainIds:Array.isArray(body.domainIds)?body.domainIds:[],orchestrationId,actor:identity.name}}));}
      ...
      const ontologyRunLinksMatch=url.pathname.match(/^\/api\/ontology\/generation-runs\/([^/]+)\/links$/);if(req.method==="POST"&&ontologyRunLinksMatch){const runId=decodeURIComponent(ontologyRunLinksMatch[1]);const existing=ontologyCandidates.getRun(runId);authorize(identity,"editor",existing.sourceId);sourceOntologyBuild.assertCatalogIdle(existing.sourceId);const run=ontologyCandidates.assertSupplementalReady(runId);return send(res,202,tasks.create({sourceId:run.sourceId,taskType:"ontology_link_generation",payload:{runId:run.id}}));}
      ...
      if(req.method==="PUT"&&url.pathname==="/api/settings"){
        ...
        // Administrators choose score-based confirmation directly. Calibration
        // remains an optional evaluation; each new run snapshots its policy.
        return send(res,200,settings.update(await readJson(req),identity.name));
      }
      ...
      if(req.method==="POST"&&url.pathname==="/api/eval/run"){const body=await readJson(req);const source=requiredSource(store,identity,body.sourceId||1,"editor");if(source.lastTestOk!==1)throw badRequest("真实数据源必须先通过只读连接测试");return send(res,202,tasks.create({sourceId:source.id,taskType:"evaluation",payload:{setName:body.setName,tolerance:body.tolerance}}));}
      if(req.method==="POST"&&url.pathname==="/api/eval/gate"){const body=await readJson(req);const source=requiredSource(store,identity,body.sourceId||1,"editor");if(source.lastTestOk!==1)throw badRequest("对照门禁需要已通过只读连接测试的真实数据源");if(body.ontologySchemaVersionId){const version=semanticSchemas.get(Number(body.ontologySchemaVersionId));if(!version||version.sourceId!==source.id)throw badRequest("候选 Schema 版本不存在或不属于当前数据源");}return send(res,202,tasks.create({sourceId:source.id,taskType:"evaluation_gate",payload:{setName:body.setName,tolerance:body.tolerance,ontologySchemaVersionId:body.ontologySchemaVersionId}}));}
      ...
      if(req.method==="GET"&&url.pathname==="/api/capability-gaps"){const sourceId=requiredSourceId(store,identity,url.searchParams.get("sourceId")||1,"editor");authorize(identity,"editor");return send(res,200,capabilityGaps.listGaps(sourceId,{limit:Math.min(500,Number(url.searchParams.get("limit")||500)),scope:url.searchParams.get("scope")==="all"?"all":"knowledge"}));}
...
function lockedSettingKeys(overrides){const keys=[];for(const group of ["llm","embedding","retrieval","discovery","profiling","ontologyAi"])if(overrides[group])for(const key of Object.keys(overrides[group]))keys.push(`${group}.${key}`);for(const key of ["queryMaxSqlCalls","queryMaxScannedRows","queryPendingTtlMs","queryMaxRows","explainMaxRows","queryTimeoutMs"])if(key in overrides)keys.push(`query.${key}`);return keys;}
