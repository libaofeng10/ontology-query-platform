...
function agentMetricsFromAnswer(answer) { const evidence=answer?.evidence||{};const fullTrace=Array.isArray(evidence.toolTrace)?evidence.toolTrace:Array.isArray(answer?.toolTrace)?answer.toolTrace:[];const trace=fullTrace.filter((item)=>item.tool!=="protocol_retry");const tokenUsage=evidence.tokenUsage||answer?.tokenUsage||null;return {agentExecution:evidence.planningMode==="agent"||answer?.planningMode==="agent"?1:0,iterations:Number(evidence.iterations||answer?.planningAttempts||0),toolCalls:trace.length,toolSuccesses:trace.filter((item)=>item.ok).length,protocolRetries:fullTrace.filter((item)=>item.tool==="protocol_retry").length,clarificationCount:answer?.clarification?1:Array.isArray(evidence.clarifications)?evidence.clarifications.length:Array.isArray(answer?.clarifications)?answer.clarifications.length:0,budgetFallback:evidence.budgetFallback?1:0,repeatedActions:trace.filter((item)=>String(item.errorCode||"").startsWith("REPEATED_ACTION")).length,intentFailures:trace.filter((item)=>item.failureClass==="intent_error").length,incompleteFailures:trace.filter((item)=>item.failureClass==="result_incomplete").length,totalTokens:tokenUsage?.available&&Number(tokenUsage.totalTokens)>0?Number(tokenUsage.totalTokens):null}; }
function createAgentGateMetrics(requestedMode) { return {gateKind:"agent",requestedMode,total:0,passed:0,failed:0,refused:0,agentExecutions:0,iterationsTotal:0,toolCalls:0,toolSuccesses:0,protocolRetries:0,clarifications:0,budgetFallbacks:0,repeatedActions:0,intentFailures:0,incompleteFailures:0,durations:[],tokenSamples:[]}; }
function addAgentGateOutcome(metrics,outcome) { metrics.total++;if(outcome.passed)metrics.passed++;else metrics.failed++;if(["join","retrieval","generation","retrieval_miss","schema_gap","policy_block","result_incomplete"].includes(outcome.failureClass))metrics.refused++;metrics.agentExecutions+=Number(outcome.agentExecution||0);metrics.iterationsTotal+=Number(outcome.iterations||0);metrics.toolCalls+=Number(outcome.toolCalls||0);metrics.toolSuccesses+=Number(outcome.toolSuccesses||0);metrics.protocolRetries+=Number(outcome.protocolRetries||0);metrics.clarifications+=Number(outcome.clarificationCount||0);metrics.budgetFallbacks+=Number(outcome.budgetFallback||0);metrics.repeatedActions+=Number(outcome.repeatedActions||0);metrics.intentFailures+=Number(outcome.intentFailures||0);metrics.incompleteFailures+=Number(outcome.incompleteFailures||0);metrics.durations.push(Number(outcome.durationMs||0));if(Number.isFinite(Number(outcome.totalTokens))&&Number(outcome.totalTokens)>0)metrics.tokenSamples.push(Number(outcome.totalTokens)); }
