...
test("semantic Gold evidence must be current, schema-bound, and produced by off-vs-prefer execution",()=>{
  ...
  const gate={id:"semantic-gate",sourceId:2,setName:"gold",total:1,ontologySchemaVersion:3,evaluationChecksum:evalSetChecksum(cases),baseline:{requestedMode:"gold",passRate:1},candidate:{requestedMode:"claude",passRate:1,claudeExecutionRate:1,joinFailureRate:0},passed:1,decision:"enable_claude"};
  ...
  const agent=inspectSemanticEvalGate({...gate,baseline:{requestedMode:"single",passRate:1},candidate:{gateKind:"agent",requestedMode:"agent_required",passRate:1,claudeExecutionRate:1}},{sourceId:2,schemaVersion:3,cases});assert.equal(agent.valid,false);assert.ok(agent.issues.some((item)=>item.code==="wrong_gate_kind"));
