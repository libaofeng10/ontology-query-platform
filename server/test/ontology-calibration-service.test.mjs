...
test("calibration report includes draft publication and downstream semantic metrics",async()=>{
  ...
  try {
    ...
    const published=fixture.store.getOntologySchemaVersion(draft.id);const evalGate=fixture.store.saveEvalGate({id:"downstream-gate",sourceId:fixture.source.id,setName:"gold",total:10,ontologySchemaVersion:draft.version,ontologySchemaPublishedAt:published.publishedAt,evaluationChecksum:evalSetChecksum(evalCases),baseline:{requestedMode:"gold",passRate:1},candidate:{requestedMode:"claude",passRate:1,claudeExecutionRate:.9,joinFailureRate:.1},passed:1,decision:"enable_claude",reason:"passed"});
    ...
    assert.equal(report.downstream.goldEquivalenceRate,1);assert.equal(report.downstream.claudeExecutionRate,.9);assert.equal(report.downstream.joinFailureRate,.1);
  } finally { fixture.store.close(); }
});
